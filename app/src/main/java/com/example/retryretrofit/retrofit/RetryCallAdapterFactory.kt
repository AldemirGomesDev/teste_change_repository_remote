...
class RetryCallAdapterFactory : CallAdapter.Factory() {
    ...
    internal class RetryCallAdapter<R, T>(
        ...
        override fun adapt(call: Call<R>): T {
            return delegated.adapt(
                if (maxRetries > 0) RetryingCall(
                    call,
                    maxRetries
                ) else call
            )
    ...
    internal class RetryCallback<T>(
        ...
        override fun onResponse(
            ...
            if (!response.isSuccessful && retryCount.incrementAndGet() <= maxRetries) {
                Log.d(
                    TAG,
                    "Call with no success result code: {} " + response.code()
                )
                retryCall()
            } else {
        ...
        override fun onFailure(call: Call<T>, t: Throwable) {
            Log.d(
                TAG,
                "Call failed with message:  " + t.message,
                t
            )
            if (retryCount.incrementAndGet() <= maxRetries) {
                ...
                callback.onFailure(
                    ...
                    TimeoutException(
                        String.format(
                            "No retries left after %s attempts.",
                            maxRetries
                        )
                    )
                )
        ...
        private fun retryCall() {
            Log.w(
                TAG,
                "" + retryCount.get() + "/" + maxRetries + " " + " Retrying..."
            )
